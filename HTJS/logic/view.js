...
import { lazyImport } from "/HTJS/helpers/lazyImport.js";
...
import { htjsUIInstance } from "../assignable/uiAssignable.js";
...
class view {
        ...
        if (!this.skipFunctionImport) {
            ...
            if (parentProxy && parentProxy._viewModuleNames && Array.isArray(parentProxy._viewModuleNames))
                for (let moduleIndex = 0; moduleIndex < parentProxy._viewModuleNames.length; moduleIndex++) {
                    let subModulePath = `/views${this.url}.${parentProxy._viewModuleNames[moduleIndex]}.js`;
                    try {
        ...
        if (this.viewModules) {
            for (var moduleIndex = 0; moduleIndex < this.viewModules.length; moduleIndex++) {
                if (typeof this.viewModules[moduleIndex] === "function") {
                    await this.viewModules[moduleIndex](this.viewModel, htjsUIInstance, baseRequestProxy.new(), this.eventProxyInstance);
                }
            }
        }
    }
    ...
    async executeViewFunction(viewFunction) {
        ...
        await viewFunction(this.viewModel, htjsUIInstance, baseRequestProxy.new(), this.eventProxyInstance);
    }
